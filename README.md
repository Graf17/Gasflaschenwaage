# Gasflaschenwaage - PlatformIO + ESPHome für Home Assistant

Dieses Projekt enthält ein PlatformIO-Testprogramm für den HX711 am Adafruit ESP32-C6 Feather
und zusätzlich eine ESPHome-Konfiguration, die für Home Assistant geeignet ist.

Ziel: Sensorik und Kalibrierung stabil zum Laufen bringen und die Daten danach in Home Assistant nutzen.
...
- `GAS_NOMINAL_KG`: Nenn-Füllmenge in kg (z. B. 5 oder 11)
- `FEATHER_I2C_SDA_PIN`, `FEATHER_I2C_SCL_PIN`: I2C-Pins für den internen MAX17048
- `FEATHER_I2C_POWER_PIN`: GPIO zum Aktivieren der I2C/STEMMA-Power (Feather: GPIO20)
- `BATTERY_UPDATE_MS`: Intervall für Batterieabfrage

Die Laufzeit-Ausgabe enthält danach:

- `raw`: Rohzählwert des HX711
- `total_kg`: gemessenes Gesamtgewicht nach Auflagenplatten-Tara
- `gas_kg`: berechnetes Gasgewicht (`total_kg - BOTTLE_EMPTY_KG`)
- `fill_pct`: berechneter Füllstand in Prozent
- `battery_v`: Akkuspannung aus MAX17048
- `battery_pct`: Akkustand aus MAX17048

Hinweis zu USB-Strom:

- Bei angeschlossenem USB wird der Akku geladen. Dadurch können `battery_v` und `battery_pct` steigen.
- Die Akku-Verbindung bleibt bestehen; der MAX17048 misst weiterhin die Zellwerte.

## Typischer Ablauf

1. Ohne Last starten und `tare` ausführen.
...
7. Neu bauen/flashen und Messwerte prüfen.
...
1. In `gasflaschenwaage.yaml` die Substitutions prüfen:
   - `bottle_empty_kg`
   - `gas_nominal_kg`
...
- Pairing über ZHA oder Zigbee2MQTT
- Bei Konfigurationsänderungen in ESPHome: Gerät im Zigbee-Netz entfernen und neu anlernen
- In Zigbee2MQTT danach Re-Interview ausführen
- Low-Power-Betrieb über Deep Sleep ist aktiviert (typisch: kurzer Wake-Zyklus, dann lange Schlafphase)

Akku/Batterie in der Konfiguration:

- `i2c_sda_pin`: I2C SDA (Feather default GPIO19)
- `i2c_scl_pin`: I2C SCL (Feather default GPIO18)
- `battery_low_pct`: Schwellwert für "Akku niedrig"
- `battery_critical_pct`: Schwellwert für "Akku kritisch"
- `awake_max_duration`: maximale Awake-Zeit pro Zyklus
- `sleep_duration`: Schlafdauer pro Zyklus

Die Messung erfolgt beim Adafruit ESP32-C6 Feather über das interne MAX17048 Fuel Gauge (I2C), nicht über einen direkten VBAT-ADC-Pin.
...
Hinweis: Auf ESP32-C6 kann Zigbee funktionieren, aber je nach Board/Funkumgebung schwanken Stabilität und Reichweite.
